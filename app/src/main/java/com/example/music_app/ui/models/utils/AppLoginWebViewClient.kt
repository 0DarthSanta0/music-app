    ...
    private fun parseResponse(url: String?) {
        ...
        with(uri) {
            ...
            if (code != null && error == null) {
                ...
                val spotifyAPI = RetrofitAuthHelper.getInstance().create(RetrofitAuthService::class.java)
                val headers = mapOf(AUTH_HEADER_NAME to auth, TYPE_HEADER_NAME to TYPE_HEADER)
                val body = TokenRequest(GRANT_TYPE, code, REDIRECT_URL)
                GlobalScope.launch {
                    try {
                        val result = spotifyAPI.getToken(headers, body).execute()
                    } catch (e: IOException) {
                        e.printStackTrace()
                    }
                }
